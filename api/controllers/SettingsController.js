...
module.exports = _.merge(_.cloneDeep(require('../base/Controller')), {
  ...
  update: function (req, res) {
    var id = req.param('id');
    sails.models.settings.update({id: id}, req.body).exec(function (err, updated) {
      if (err) return res.negotiate(err);
      var setting = updated[0];
      if (setting) {
        sails.KONGA_CONFIG = setting.data || {};
        EventService.broadcast(req, 'setting', 'update', setting);
      }
