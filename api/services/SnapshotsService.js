...
module.exports = {
  ...
  snapshot: async(snapshotName, node) => {
    ...
    try {
      ...
      const formattedVersion = Utils.ensureSemverFormat(nodeInfo.version);

      if (semver.gte(formattedVersion, '3.0.0')) {
        entities['vaults'] = [];
        entities['keys'] = [];
        entities['key-sets'] = [];
      }
  ...
  takeSnapShot: function (name, node, cb) {
    ...
    KongService.nodeInfo(node, function (err, status) {
      ...
      var endpoints = ['/plugins', '/consumers']

      status.version = Utils.ensureSemverFormat(status.version);

      if (semver.lt(status.version, '1.0.0')) {
        endpoints = endpoints.concat(['/apis']);
      }
      ...
      if (semver.gte(status.version, '3.0.0')) {
        endpoints = endpoints.concat(['/vaults', '/keys', '/key-sets']);
      }
