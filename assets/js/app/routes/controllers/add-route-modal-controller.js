      ...
      function controller(_, $scope, $rootScope, $log, $state, RoutesService, SettingsService,
        ...
        $scope.submit = function () {
          ...
          RoutesService.add(data)
            ...
            var originalMessage = _.get(err,'data.body.message',"");
            var friendlyMessage = "Submission failed. " + originalMessage;
            if (originalMessage && (originalMessage.indexOf("at least one of hosts, paths") > -1 || originalMessage.indexOf("schema violation") > -1)) {
              friendlyMessage = "Submission failed: You must specify at least one routing rule (e.g. at least one 'Path' like /my-path, 'Host', 'Method', or 'Header') so Kong knows how to match incoming requests to this service.";
            }
            MessageService.error(friendlyMessage);
            $scope.errors = {}
            ...
            if (errorBody) {
              ...
              $scope.errorMessage = (originalMessage.indexOf("at least one of hosts, paths") > -1 || originalMessage.indexOf("schema violation") > -1) ? friendlyMessage : (errorBody.message || friendlyMessage);
            } else {
              $scope.errorMessage = friendlyMessage || "An unknown error has occured"
            }
