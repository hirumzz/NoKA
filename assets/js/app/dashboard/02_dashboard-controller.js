      ...
      function controller($scope,$rootScope, $log, $state,$q,InfoService,$localStorage,HttpTimeout, $location,
          ...
          function drawCharts() {
              $scope.data = {
                  ...
                  topHits: {
                      labels: $scope.prometheus && $scope.prometheus.topHits ? $scope.prometheus.topHits.map(function(item) { return item.endpoint; }) : [],
                  ...
                  slowestEndpoints: {
                      labels: $scope.prometheus && $scope.prometheus.slowestEndpoints ? $scope.prometheus.slowestEndpoints.map(function(item) { return item.endpoint; }) : [],
          ...
          function fetchData() {
              ...
              var prometheus = InfoService
                  .getPrometheusMetrics()
                  .then(function(resp){
                      if (resp.data && resp.data.success) {
                          $scope.prometheus = resp.data;
                          $scope.metricsEnabled = true;
                      } else {
                          $scope.metricsEnabled = false;
                      }
                      $log.info("DashboardController:fetchData:prometheus",$scope.prometheus)
                  })
                  .catch(function(err){
                      $scope.metricsEnabled = false;
                      $log.error("DashboardController:fetchData:prometheus error", err);
                  });

              $q
                  .all([status, info, prometheus])
                  .finally(
                      function onFinally() {
                          $scope.loading = false
