    ...
    .controller('ServicesController', [
      ...
      function controller($scope, $rootScope, $log, $state, ServiceService, ListConfig, ServiceModel,
        ...
        $scope.filterByTag = function (item) {
          ...
          var hasTag = false;
          if (item.tags && item.tags.indexOf($scope.selectedTag) > -1) {
            hasTag = true;
          }
          if (item.extras && item.extras.tags && item.extras.tags.indexOf($scope.selectedTag) > -1) {
            hasTag = true;
          }
          return hasTag;
        };
          ...
          ServiceModel.load({
            ...
            if ($scope.items && $scope.items.data) {
              $scope.items.data.forEach(function(service) {
                if (service.tags) {
                  service.tags.forEach(function(t) { tagsMap[t] = true; });
                }
                if (service.extras && service.extras.tags) {
                  service.extras.tags.forEach(function(t) { tagsMap[t] = true; });
                }
              });
            }
